...
func _physics_process(delta: float) -> void:
	...
	if d > range:
		var new_vel := Vector2(speed, 0).rotated(global_position.direction_to(player.global_position).angle())
		new_vel.y += sin(osc) * speed * sin_ratio
		...
		if randf() <= 0.01 and osc > PI:
			rand_vel = Vector2(randf_range(-speed * rand_ratio, speed * rand_ratio), randf_range(-speed * rand_ratio, speed * rand_ratio))
		var new_vel := Vector2(rand_vel.x, sin(osc) * speed * sin_ratio + rand_vel.y)
		var d_v := new_vel - entity.velocity
